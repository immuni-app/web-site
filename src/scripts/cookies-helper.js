...
export function handleCookies() {
  const addCookieBanner = () => {
    ...
    bannerCookie.innerHTML = `<div id="cookie-banner" class="dismissed"><div><span>${message[lang].message}</span><a href="/pn.html" target="_blank" class="pn-link">${message[lang].policy}</a><button class="button cookies" id="accept-btn">${message[lang].button}</button></div>`;
...
export function updateCookiesLang() {
  const lang = localStorage.getItem("language");
  const cookieBanner = document.getElementById("cookie-banner");
  cookieBanner.innerHTML = `<div><span>${message[lang].message}</span><a href="/pn.html" target="_blank" class="pn-link">${message[lang].policy}</a><button class="button cookies" id="accept-btn">${message[lang].button}</button>`;
  const acceptButton = document.getElementById("accept-btn");
  acceptButton.addEventListener("click", okWithCookies);
}
